...
###############################################################################
# RBAC: Allow the connector to read/write blobs in the storage account
###############################################################################
resource "azurerm_role_assignment" "uc_storage_blob_contrib" {
  scope                = azurerm_storage_account.datalake.id  # ✅ Changed from container to storage account
